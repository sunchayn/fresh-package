...
- `src/Http/Api/{Controllers,Requests,Resources}`: HTTP concerns for the package's API surface, loaded through `routes/api.php`.
- `src/Http/Web/Controllers`: HTTP concerns for browser-facing routes, loaded through `routes/web.php` (and, when the frontend is kept, the SPA entry route).
