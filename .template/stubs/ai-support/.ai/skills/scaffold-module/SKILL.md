...
5. Add an HTTP transport layer only if the capability or module is reachable over HTTP: a controller under `src/Http/Api/Controllers` or `src/Http/Web/Controllers`, its request/resource pair for an API endpoint, and the route in `routes/api.php` or `routes/web.php`. Console-only or internal-only work skips this entirely.
...
- To add an API endpoint, put the controller under `src/Http/Api/Controllers`, the request under `src/Http/Api/Requests`, and the resource under `src/Http/Api/Resources`. Add the route in `routes/api.php`. Keep any non-trivial business logic in `src/Modules/<Domain>` rather than the controller.
