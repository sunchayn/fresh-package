...
2. If a PHP file changed under one of these folders, `src/`, `tests/`, `config/`, `database/`<!-- @chisel-workbench -->, `workbench/`<!-- @end-chisel-workbench -->, run in order: `composer analyse`, `composer rector`, `composer style:fix`, then `composer test:parallel` filtered to the affected area while iterating, and `composer test:parallel` before finishing.
...
<!-- @chisel-vue -->
4. If `resources/js` changed, run `npm run style:check`, `npm run type:check`, and `npm run test`.
<!-- @end-chisel-vue -->
